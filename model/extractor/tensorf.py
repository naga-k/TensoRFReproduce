...
import numpy as np
import time as time
from .tensorf_sh import eval_sh_bases
...
def raw2alpha(sigma, dists):

    #NOTE: Gathering weights for accumulated tranmittance

    alpha = 1. - torch.exp(-sigma * dists)
    T = torch.cumprod(torch.cat([torch.ones(alpha.shape[0], 1).to(alpha.device), 1. - alpha + 1e-10], -1), -1)

    weight = alpha * T[:, :-1]
    return alpha, weight, T[:, -1:]
...
class TensorVMSplit(torch.nn.Module):
    def __init__(self, aabb, gridSize, device, density_n_comp=8, appearance_n_comp=24, app_dim=27,
        ...
        #NOTE: For more on feature2densityshift, refer to self.feature2density
    ...
    def save(self, path):
        ...
        ckpt.update({'iteration': self.iteration})
    ...
    def load(self, ckpt):
        ...
        if 'alphaMask.aabb' in ckpt.keys():
            ...
            print("alpha mask shape", ckpt['alphaMask.shape'])
        if 'iteration' in ckpt.keys():
            self.iteration = ckpt['iteration']
        
            print("model iteration at ckpt", self.iteration)
    ...
    def sample_ray(self, rays_o, rays_d, is_train=True, N_samples=-1):
        ...
        # print("sample ray shapes")
        # print(vec.shape, rays_o.shape, self.aabb.shape)
